...
-- Consultar a soma de cargas horárias
...
SELECT 
    t.id, -- > ID da tabela de turmas, ou seja, o código de turma
    p.nome, -- > Nome do professor na tabela de professores
    t.data_inicio
    from turmas AS t
    -- INNER JOIN nome_tabela AS apelido ON (PK = FK)
    INNER JOIN professores AS p ON (p.id = t.id_professor);
    -- TODO: adicionar consulta para nome do professor;

-- Consultar o nome do curso, a data de início e término
SELECT 
...
-- Consultar o nome do professor e nome do curso, data de início e término

SELECT
    professores.nome AS "Professor",
    cursos.nome AS "Curso",
    turmas.data_inicio AS "Data de início",
    turmas.data_termino AS "Data de término"
    FROM turmas
...
SELECT
    ...
    UPPER(cursos.nome) AS "Curso",          -- Nome do curso em caix alta
    turmas.data_inicio AS "Data de início",
    turmas.data_termino AS "Data de término"
    FROM turmas
    INNER JOIN professores ON(professores.id = turmas.id_professor)
...
-- Consultar as mercadorias onde o nome terminal com nescau
SELECT * FROM mercadorias WHERE nome LIKE "%Nescau"; -- 2, 7

-- Consultar as mercadorias onde o nome contém nescau (começo, meio, fim)
...
-- Consultar a nome das mercadorias substituindo o Nescal por Nescau
...
SELECT LTRIM(nome), LENGTH(RTRIM(nome)) from mercadorias;

-- Consultar o nome removendo os espaços da direita e esquerda
SELECT LTRIM(nome), LENGTH(TRIM(nome)) from mercadorias;

-- Adicionar coluna de data de vencimento na tabela mercadorias
...
-- Definir o valor padrão para a coluna da data de vencimento do produto
...
ALTER TABLE mercadorias CHANGE valor_unitario preco_unitario FLOAT(5, 4);
...
-- Adicionar constraint de validação de idade, onde a idade deve ser maior ou igual a 14 anos
...
UPDATE professores SET cpf = '531.465.650-72' WHERE id = 1;
UPDATE professores SET cpf = '373.010.360-19' WHERE id = 2;
UPDATE professores SET cpf = '578.629.670-97' WHERE id = 3;
UPDATE professores SET cpf = '326.525.660-08' WHERE id = 4;
...
INSERT INTO professores (nome, valor_hora, cpf) VALUE('Johana', 197.50, '474.368.310-62');

-- Apagar a professora Johana pois a tabela n tinha validação de cpf único
...
-- Consultar o nome dos produtos do mercado uma única vez, ou seja, o nome não se repetirá na consulta
...
ALTER TABLE mercadorias MODIFY preco_unitario FLOAT(5,2);
ALTER TABLE mercadorias ADD COLUMN corredor INT;
...
SELECT categoria, SUM(preco_unitario) FROM mercadorias;
...
-- Gerar JSON com aas propriedades utilizando função do MYSQL
...
-- Criado um procedure para concatenar o nome e sobrenome, apresentando ao usuário
...
DELIMITER $
CREATE PROCEDURE CadastrarAlunoComDataIdade(IN nome VARCHAR(30), IN idade INT, IN mes INT, IN dia INT)
BEGIN
    DECLARE ano_nascimento INT;
    DECLARE ano_hoje INT;
    SET ano_nascimento := YEAR(data_nascimento);
